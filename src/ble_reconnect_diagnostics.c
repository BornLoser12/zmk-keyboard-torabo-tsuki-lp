...
static const struct device *const diagnostics_uart = DEVICE_DT_GET(DT_NODELABEL(ble_reconnect_uart));

static bool diagnostics_uart_ready(void) {
    uint32_t dtr = 0;

    return device_is_ready(diagnostics_uart) &&
           uart_line_ctrl_get(diagnostics_uart, UART_LINE_CTRL_DTR, &dtr) == 0 && dtr;
}

static void diagnostics_print(const char *format, ...) {
    if (!diagnostics_uart_ready()) {
        return;
    }
    ...
    int length = vsnprintk(buffer, sizeof(buffer), format, args);
    va_end(args);
    ...
    for (int i = 0; i < length; i++) {
        uart_poll_out(diagnostics_uart, buffer[i]);
    }
    uart_poll_out(diagnostics_uart, '\r');
    uart_poll_out(diagnostics_uart, '\n');
}

#define DIAG_PRINT(...) diagnostics_print(__VA_ARGS__)
...
static void log_address(const char *label, const bt_addr_le_t *addr) {
    if (!addr) {
        DIAG_PRINT("BLE_DIAG addr %s=<null>", label);
        return;
    }

    char addr_str[BT_ADDR_LE_STR_LEN];
    bt_addr_le_to_str(addr, addr_str, sizeof(addr_str));
    DIAG_PRINT("BLE_DIAG addr %s=%s type=%u raw=%02x:%02x:%02x:%02x:%02x:%02x", label,
...
static void dump_profiles(const bt_addr_le_t *dst, const bt_addr_le_t *remote) {
    ...
    DIAG_PRINT("BLE_DIAG profiles active=%d dst_index=%d remote_index=%d count=%d", active,
    ...
    for (uint8_t i = 0; i < ZMK_BLE_PROFILE_COUNT; i++) {
        ...
        DIAG_PRINT("BLE_DIAG profile[%u] addr=%s open=%u active=%u cmp_dst=%d cmp_remote=%d", i,
...
static void dump_bond(const struct bt_bond_info *info, void *user_data) {
    ...
    DIAG_PRINT("BLE_DIAG bond[%u] addr=%s cmp_dst=%d cmp_remote=%d", context->count, addr_str,
...
static void dump_bonds(const bt_addr_le_t *dst, const bt_addr_le_t *remote) {
    ...
    DIAG_PRINT("BLE_DIAG bonds total=%u", context.count);
}
...
static void dump_connection(const char *event, struct bt_conn *conn) {
    ...
    if (err) {
        DIAG_PRINT("BLE_DIAG event=%s conn=%u get_info_err=%d", event, bt_conn_index(conn), err);
    ...
    DIAG_PRINT("BLE_DIAG event=%s conn=%u role=%u state=%u id=%u security=%u key_size=%u "
...
static void snapshot_work_handler(struct k_work *work) {
    ...
    DIAG_PRINT("BLE_DIAG snapshot begin uptime_ms=%lld", k_uptime_get());
    dump_profiles(NULL, NULL);
    dump_bonds(NULL, NULL);
    DIAG_PRINT("BLE_DIAG snapshot end");
}
...
static void diagnostics_connected(struct bt_conn *conn, uint8_t err) {
    DIAG_PRINT("BLE_DIAG callback connected err=0x%02x", err);
    dump_connection("connected", conn);
}

static void diagnostics_disconnected(struct bt_conn *conn, uint8_t reason) {
    DIAG_PRINT("BLE_DIAG callback disconnected reason=0x%02x", reason);
...
static void diagnostics_identity_resolved(struct bt_conn *conn, const bt_addr_le_t *rpa,
    ...
    DIAG_PRINT("BLE_DIAG callback identity_resolved");
...
static void diagnostics_security_changed(struct bt_conn *conn, bt_security_t level,
    ...
    DIAG_PRINT("BLE_DIAG callback security_changed level=%u err=%u", level, err);
...
static void diagnostics_pairing_complete(struct bt_conn *conn, bool bonded) {
    DIAG_PRINT("BLE_DIAG callback pairing_complete bonded=%u", bonded);
...
static void diagnostics_pairing_failed(struct bt_conn *conn, enum bt_security_err reason) {
    DIAG_PRINT("BLE_DIAG callback pairing_failed reason=%u", reason);
    dump_connection("pairing_failed", conn);
}

static void diagnostics_bond_deleted(uint8_t id, const bt_addr_le_t *peer) {
    DIAG_PRINT("BLE_DIAG callback bond_deleted id=%u", id);
